...
//php pour php_frc.h
char* process_php_tags(const char* html_input, const char* buffer) {
    const char* start = html_input;
    char* final_output = malloc(1);
    size_t final_size = 0;
    final_output[0] = '\0';

    while ((start = strstr(start, "<?php")) != NULL) {
        // Ajouter le texte avant le <?php
        ...
        final_output = realloc(final_output, final_size + pre_len + 1);
        strncat(final_output + final_size, html_input, pre_len);
        final_size += pre_len;
        final_output[final_size] = '\0';

        // Extraire le code PHP
        size_t code_len = code_end - code_start;
        char* php_code = malloc(code_len + 1);
        strncpy(php_code, code_start, code_len);
        php_code[code_len] = '\0';

        // Appeler PHP
        // On ajoute les balises PHP manquantes autour du code extrait
        ...
        if (php_result) {
            ...
            strcpy(final_output + final_size, php_result);
    ...
    char* final_output_html;
    final_output_html = strdup(final_output);
    free(final_output);
    return final_output_html;
}
...
void handle_client(SOCKET client_socket) {
    ...
    (ext_ptr = strstr(decoded_path, ".frc")) && (!query_start || ext_ptr < query_start)) {
        ...
        send(client_socket, header, strlen(header), 0);
        send(client_socket, buffer1, len, 0);
        free(buffer1);
        closesocket(client_socket);
        return;
    }
    ...
    //pour php_frc.h
    if (info_ip_data[0]){
        char* resu = set_client_ip(info_ip_data);
        if (strstr(resu,"1")==0){
            printf("ERREUR PHP !\n");
            fflush(stdout);
        }
