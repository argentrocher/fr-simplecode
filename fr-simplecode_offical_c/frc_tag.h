...
int get_tag(const char *key, double *value){
    if (!key || !value)
        return 0;
        ...
        for (size_t j = 0; j < system_tags[i].number_key; j++){
            if (strcmp(system_tags[i].key[j], key) == 0) {
                ...
                if (system_tags[i].type_value == 0) {
                    int tmp = *(int *)system_tags[i].value;
                    *value = (double)tmp;
    ...
    for (size_t i = 0; i < user_tags_count; i++) {
        if (strcmp(user_tags[i].key, key) == 0) {
            *value = user_tags[i].current_value;
            //printf("GET tag[%zu] key=%s ptr=%p value=%d\n", i, user_tags[i].key, (void *)&user_tags[i].current_value, user_tags[i].current_value);
    ...
    for (size_t i = 0; i < (sizeof(system_tags)/sizeof(system_tags[0])); i++) {
        for (size_t j = 0; j < system_tags[i].number_key; j++) {
            ...
            for (size_t k = 0; k < system_tags[i].number_states; k++) {
                ...
                int tag_value = (int)system_tags[i].states[k].value;
    ...
    for (size_t i = 0; i < user_tags_count; i++) {
        ...
        for (size_t j = 0; j < user_tags[i].number_states; j++) {
            if (strcmp(user_tags[i].states[j].name, state) == 0) {
                //printf("SET tag[%zu] key=%s ptr=%p old=%d new=%d\n", i, user_tags[i].key, (void *)&user_tags[i].current_value, user_tags[i].current_value, user_tags[i].states[j].value);
                user_tags[i].current_value = user_tags[i].states[j].value;
                //printf("SET AFTER tag[%zu] key=%s ptr=%p old=%d new=%d\n", i, user_tags[i].key, (void *)&user_tags[i].current_value, user_tags[i].current_value, user_tags[i].states[j].value);
...
static int parse_create_tag(const char **cursor, const char *end) {
    ...
    // valeur utiliser pour les ++ et -- dans la structure, reprend la valeur précédente et ajoute ou retire 1 pour la nouvelle valeur
    double last_value_option = 1;
    ...
    while (p < end) {
        ...
        double double_value = (double)states_count+1;
        if (*p == '=') {
            //value
            p++;
            while (p < end && is_space(*p)) p++;
            ...
            if (strncmp(expr_start, "++", expr_len)==0) {
                double_value = last_value_option+1;
            } else if (strncmp(expr_start, "--", expr_len)==0) {
                double_value = last_value_option-1;
            } else {
                char *expr = malloc(expr_len+1+100); //marge car eval veux de la place pour modifier
                if (!expr) {
                    free(key);
                    for (size_t i = 0; i < states_count; i++)
                        free((char *)states[i].name);
                ...
                double_value = eval_option(expr, &just_value);
                ...
                if (just_value) {
                    ...
                    for (size_t i = 0; i < states_count; i++)
                        free((char *)states[i].name);
                    free(states);
                    error_handler("Erreur : des valeurs simples sont attendu sans fonctions ni variables sur ?tag.nametag{...}.", "Error: simple values are expected without functions or variables on ? tag.nametag{...}.", ERROR_MODE_CARRIAGE_RETURN);
                    return 0;
                }
            }
        }
        ...
        states[states_count].value = double_value;

        //save ancienne valeur pour ++ --
        last_value_option = double_value;
