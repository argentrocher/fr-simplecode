...
#ifndef TABLE_SIZE
#define TABLE_SIZE 16384 //! doit être une puissance de 2
#endif
...
// avec error_handler() pour les erreurs
...
_Thread_local HashTable* environments = NULL;  // Liste des environnements (global, local, etc.)
...
void push_env_object(const char* env_name) {
    if (env_stack_top + 1 < MAX_ENV_STACK) {
        ...
        error_handler("Erreur: pile d'environnements pleine !\n", "Error: stack of environments full !\n", ERROR_MODE_DEFAULT);
    }
}
...
void set_env_object(const char* env_name, const char* var, const char* value) {
    ...
    while (curr) {
        if (curr->hash == h && strcmp(curr->key, var) == 0) {
            free(curr->value);
...
const char* get_env_object(const char* env_name, const char* var) {
    ...
    return NULL; //"Environnement non trouvé.";
}
...
void pop_env_object() {
    if (env_stack_top >= 0) {
        ...
        error_handler("Erreur: aucun environnement à retirer.\n", "Error: no environment to remove.\n", ERROR_MODE_DEFAULT);
    }
}
...
void set_table_env(const char* env_name, const char* table_name, const char* content) {
    HashTable* env = find_or_create_env(env_name);
    unsigned int h = hash(table_name);
    unsigned int idx = h & (TABLE_SIZE - 1);
    Entry* curr = env->buckets_tables[idx];

    while (curr) {
        if (curr->hash == h && strcmp(curr->key, table_name) == 0) {
            free(curr->value);
...
//générer un nom de table pas encore utilisé pour la copie pour les fonctions
char* generate_unique_table_name(const char* env_name) {
    static char buffer[512];
    int index = 0;
    bool exists = true;

    while (exists) {
        snprintf(buffer, sizeof(buffer), "temp%d", index);
        exists = false;
...
int copy_table_between_envs(
    const char* src_table_name,
    const char* src_env,
    const char* dest_table_name,
    const char* dest_env
) {
    const char* src_content = get_table_env(src_env, src_table_name);
    if (!src_content || strcmp(src_content, "Table non trouvée.") == 0 || strcmp(src_content, "Environnement non trouvé.") == 0) {
        //fprintf(stderr, "Erreur : table '%s' introuvable dans l'environnement '%s'.\n", src_table_name, src_env);
        return 0;
    }

    // Copie sécurisée du contenu dans le nouvel environnement
    set_table_env(dest_env, dest_table_name, src_content);
