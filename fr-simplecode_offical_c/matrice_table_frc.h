...
static char *expand_table_references_matrix(const char *expr) {
    ...
    while (*p) {
        const char *pos = strstr(p, "table|");
        if (!pos) {
            // Plus de table| -> copier le reste
        ...
        if (name_len == 0 || name_len > 255) {
            // Nom vide ou trop long -> ignorer
            p = end + 1;
            continue;
        }
        char table_name[256];
        strncpy(table_name, pos, name_len);
        ...
        if (!skip_replace) {
            ...
            if (read_table_inner_from_file(table_name, table_content, sizeof(table_content))) {
                // On a trouvé la table -> remplacer par [[contenu]]
                ...
                out_len += written; //add_len - 1
                p = end + 1;
                continue;
            }
        }

        // Pas de table trouvée ou sautée -> recopier "table|nom|"
