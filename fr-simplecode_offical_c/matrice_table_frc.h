...
// ===========================================================
// FONCTIONS DE PARSING NUMÉRIQUE POUR LES PARENTHÈSES ET PRIORITÉS
// ===========================================================

static double parse_expression_matrix(const char **p);
static double parse_term_matrix(const char **p);
static double parse_factor_matrix(const char **p);
static int compute_values_matrix_mode(const char *expr, char *result_out, size_t outsz);

static double parse_number_matrix(const char **p) {
    char buf[128];
    size_t i = 0;
    skip_spaces(p);
    while ((**p >= '0' && **p <= '9') || **p == '.' || **p == ',') {
        if (i + 1 < sizeof(buf)) buf[i++] = **p;
        (*p)++;
    }
    buf[i] = '\0';
...
static double parse_factor_matrix(const char **p) {
    ...
    if (**p == '(') {
        (*p)++;
        double val = parse_expression_matrix(p);
        skip_spaces(p);
        if (**p == ')') (*p)++;
        return val;
    } else {
        return parse_number_matrix(p);
    }
}

static double parse_term_matrix(const char **p) {
    double val = parse_factor_matrix(p);
    skip_spaces(p);
    while (**p == '*' || **p == '/' || **p == '%' || **p == '#') {
        char op = **p;
        (*p)++;
        double rhs = parse_factor_matrix(p);
        switch (op) {
            case '*': val *= rhs; break;
            case '/': val /= rhs; break;
            case '%': val = fmod(val, rhs); break;
            case '#': val = floor(val / rhs); break;
        }
        skip_spaces(p);
    }
    return val;
...
static double parse_expression_matrix(const char **p) {
    double val = parse_term_matrix(p);
    skip_spaces(p);
    while (**p == '+' || **p == '-') {
        char op = **p;
        (*p)++;
        double rhs = parse_term_matrix(p);
        if (op == '+') val += rhs;
        else val -= rhs;
        skip_spaces(p);
    }
...
static int compute_values_matrix_mode(const char *expr, char *result_out, size_t outsz) {
    ...
    const char *pp = insert_hidden_multiplications_matrix(p);
    if (!pp){
    ...
    char *res = eval_matrix_expression(&pp, 0);
