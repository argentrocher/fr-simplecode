...
static HANDLE variable_save_thread_handle = NULL;
static HANDLE variable_save_event = NULL;

static volatile LONG variable_save_shutdown = 0;
...
static DWORD WINAPI variable_save_thread(void *arg) {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            /*
             * Récupération atomique de l'entrée à sauvegarder.
             */
            EnterCriticalSection(&variable_cache_cs);
             ...
             *
             * On ne quitte PAS immédiatement :
             * on termine d'abord toutes les entrées dirty.
             */
             ...
             * Échec d'allocation.
             *
             * On ne marque surtout pas l'entrée comme propre.
             */
            if (!name || !value) {
                if (!name) free(name);
                if (!value) free(value);
             ...
             *
             * IMPORTANT :
             * ne jamais garder variable_cache_cs pendant cette
             * opération, sinon le thread principal serait bloqué
             * pendant tout le CreateFile/MapView/WriteFile.
             */
             ...
             *
             * Si la génération est identique, la valeur écrite
             * correspond toujours à la dernière valeur connue.
             */
            if (index >= 0 &&
...
void cache_clean_variables(void) {
    ...
    /*
     * Le réveiller même s'il dort.
     */
    SetEvent(variable_save_event);

    /*
     * Le thread va :
     *   - terminer toutes les entrées dirty
     *   - puis retourner
     */
    ...
    /*
     * Maintenant seulement, le thread est terminé,
     * donc le cache peut être libéré sans verrou.
     */
    EnterCriticalSection(&variable_cache_cs);
